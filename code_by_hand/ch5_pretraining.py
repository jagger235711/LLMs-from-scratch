...
# %%
# GPT 预测下一个词时，会输出一个包含所有词汇概率的列表（logits）。


# 不加 Top-k：从所有词里挑，可能选出一个很生僻但概率略高的词，导致文不对题。
# 加上 Top-k：只保留概率最高的前 k 个词，把剩下所有词的概率直接变成 −∞（负无穷）。
def generate(
...
NEW_CONFIG = GPT_CONFIG_124M.copy()  # 这个124M是单个配置
# print(NEW_CONFIG)
NEW_CONFIG.update(
    model_configs[model_name]
)  # 把model_configs[model_name]中的键值对更新到NEW_CONFIG中
# print(NEW_CONFIG)
...
def assign(left, right):
    # 检查两个张量或数组(left 和 right) 是否具有相同的维度或形状,并返回可训练的 PyTorch 参数
